...
const initialContacts = [
  { id: 101, firstName: 'Jane', lastName: 'Smith', email: 'jane.smith@example.com', service: 'weddings', message: 'Hi, I am looking for a wedding photographer for next spring. Can you share some premium packages?', date: 'Oct 24, 2026', status: 'Pending', notes: '' },
  { id: 102, firstName: 'Michael', lastName: 'Johnson', email: 'michael.j@example.com', service: 'corporate', message: 'We need coverage for our annual company retreat. Looking forward to hearing from you.', date: 'Oct 25, 2026', status: 'In Progress', notes: 'Emailed pricing PDF on Oct 26.' },
  { id: 103, firstName: 'Sarah', lastName: 'Williams', email: 'sarah.w@example.com', service: 'portraits', message: 'Do you do family portraits at outdoor locations?', date: 'Oct 26, 2026', status: 'Resolved', notes: 'Booked a session for Nov 5.' },
];
...
const Contact = () => {
  ...
  const [contacts, setContacts] = useState(() => {
    const saved = localStorage.getItem('drushya_contacts');
    if (saved) return JSON.parse(saved);
    return initialContacts;
  });

  useEffect(() => {
    localStorage.setItem('drushya_contacts', JSON.stringify(contacts));
  }, [contacts]);
  ...
  const handleSave = () => {
    if (modalMode === 'edit' && selectedContact) {
      setContacts(contacts.map(c => 
        c.id === selectedContact.id ? { ...c, status: formData.status, notes: formData.notes } : c
      ));
    }
    closeModal();
  };
                    ...
                    <tr key={contact.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="font-bold text-gray-900">{contact.firstName} {contact.lastName}</div>
                      ...
                      <td className="py-4 px-6 text-gray-600">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="whitespace-nowrap">{contact.date}</span>
                        </div>
                      </td>
                      <td className="py-4 px-6">
          ...
          {selectedContact && (
            <div className="flex-1 overflow-y-auto pr-2">
              <div className="bg-gray-50 p-4 rounded-xl mb-6">
                ...
                <div className="font-bold text-gray-900 text-lg mb-2">{selectedContact.firstName} {selectedContact.lastName}</div>
              ...
              <div className="bg-yellow-50/50 border border-yellow-100 p-4 rounded-xl mb-6">
                <div className="text-sm text-yellow-800 font-semibold mb-2 flex justify-between">
                  ...
                  <span className="text-gray-500 font-normal">{selectedContact.date}</span>
                </div>
                <div className="flex flex-col gap-3 text-sm text-gray-700">
                  <div>
                    <strong className="block mb-1 text-gray-900">Service of Interest:</strong> 
                    <span className="bg-white px-2 py-1 rounded border border-gray-200 inline-block">{getServiceName(selectedContact.service)}</span>
                  </div>
                  <div>
